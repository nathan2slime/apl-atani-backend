...
A NestJS backend for APL Atani, providing cookie-based authentication, user lookup, threaded posts with file attachments, health checks, and generated API documentation.

PostgreSQL is the source of truth through Prisma, Redis stores authenticated session data, and an S3-compatible object store keeps uploaded post assets. The local Docker Compose environment provides PostgreSQL, Redis, MinIO, and the API container.
...
|-- docker-compose.yaml # API, database, cache and storage stack
...
| [`docker-compose.yaml`](docker-compose.yaml) | Local API, PostgreSQL, Redis and MinIO stack |
...
The Docker image starts the compiled API with:

```sh
pnpm start:prod
```

Run deploy migrations explicitly when preparing a production-like environment:
...
| Local infrastructure | Reproducible PostgreSQL, Redis and MinIO services through Docker Compose |
